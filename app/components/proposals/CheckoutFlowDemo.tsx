...
type ActivePanel = "customer" | "owner" | "dashboard";
...
export default function CheckoutFlowDemo({
  ...
  const [activePanel, setActivePanel] = useState<ActivePanel>("customer");
  const [ownerNotifStep, setOwnerNotifStep] = useState(0); // 0=none, 1=payment, 2=crm
  const [dashRowStep, setDashRowStep] = useState(0); // 0=none, 1=payment, 2=crm
  ...
  const handlePay = useCallback(() => {
    ...
    addTimer(() => {
      setStep(4); // Confirmation on customer phone

      // After confirmation shows for 2s, switch to owner phone
      addTimer(() => {
        setActivePanel("owner");
        // Show payment notification after panel transition
        addTimer(() => {
          setOwnerNotifStep(1);
          // Show CRM notification
          addTimer(() => {
            setOwnerNotifStep(2);
            // After both notifs shown for 1.5s, switch to dashboard
            addTimer(() => {
              ...
              const counterTimer = setInterval(() => {
                ...
                if (s >= steps) clearInterval(counterTimer);
              }, interval);
              timersRef.current.push(counterTimer as unknown as ReturnType<typeof setTimeout>);
          ...
          <div className="relative h-full w-full">
            ...
            >
              ...
              <div className="flex items-center gap-1.5 mt-4">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                <span className="text-[9px] text-text-dim">Secured by Stripe</span>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                <div className="px-3 py-2.5 rounded-xl border border-white-10 bg-white-6">
                  <p className="text-[9px] text-text-dim mb-0.5">Card Number</p>
                  <p className="text-[12px] text-text-mid font-mono">4242 •••• •••• 4242</p>
                </div>
                <div className="flex gap-2">
                  <div className="flex-1 px-3 py-2.5 rounded-xl border border-white-10 bg-white-6">
                    <p className="text-[9px] text-text-dim mb-0.5">Expiry</p>
                    <p className="text-[12px] text-text-mid font-mono">12/27</p>
                  </div>
                  <div className="flex-1 px-3 py-2.5 rounded-xl border border-white-10 bg-white-6">
                    <p className="text-[9px] text-text-dim mb-0.5">CVC</p>
                    <p className="text-[12px] text-text-mid font-mono">•••</p>
                  </div>
                </div>
                <div className="px-3 py-2.5 rounded-xl border border-white-10 bg-white-6">
                  <p className="text-[9px] text-text-dim mb-0.5">Cardholder Name</p>
            ...
            >
              ...
              <p className="text-[13px] font-semibold text-text">Processing payment...</p>
              <p className="text-[10px] text-text-dim mt-1">Please wait</p>
            </div>
            ...
            >
              <div className="w-14 h-14 rounded-full bg-accent/20 flex items-center justify-center mb-4">
                {step >= 4 && (
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="20 6 9 17 4 12" className="checkout-check-draw" />
                  </svg>
                )}
              </div>
              <p className="text-[16px] font-bold text-text mb-1">Payment Successful!</p>
              <p className="text-[11px] text-text-dim text-center leading-[1.6]">
                ${price} charged to •••• 4242<br />
              ...
              <div className="mt-5 w-full rounded-xl border border-white-10 bg-white-6 p-3">
                <div className="flex justify-between text-[10px] mb-1.5">
                  ...
                  <span className="text-text font-semibold">{packages[selectedPkg].name}</span>
                </div>
                <div className="flex justify-between text-[10px] mb-1.5">
                  <span className="text-text-dim">Amount</span>
                  <span className="text-text font-semibold">${price}/mo</span>
                </div>
  ...
  const ownerPhone = (
    <div
      className={`relative mx-auto shrink-0 ${ownerNotifStep >= 1 ? "phone-vibrate" : ""}`}
      style={{ width: 260, height: 540 }}
    >
      <div className="absolute inset-0 rounded-[36px] border-2 border-white-15 bg-[#0a0a0a] overflow-hidden shadow-[0_0_60px_rgba(0,0,0,0.5)]">
        {/* Notch */}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-start pt-14 px-4">
          ...
          <p className="text-[11px] text-text-dim mt-1">Wednesday, February 26</p>
        </div>
        ...
        >
          <div className="flex items-center gap-2.5 mb-1.5">
            ...
            <div>
              <p className="text-[11px] font-bold text-text">New Payment: ${price}</p>
              <p className="text-[10px] text-accent">from Sarah M. · Just now</p>
            </div>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] font-bold text-text">CRM Automation</p>
              <p className="text-[9px] text-text-dim">Sarah M. added to onboarding workflow</p>
            </div>
          </div>
        </div>
      </div>
   ...
   * ============================================================= */
  const dashboard = (
    <div className="w-full max-w-[320px] mx-auto bg-surface border border-border rounded-2xl p-5">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <p className="text-[13px] font-bold text-text">{clientBusinessName}</p>
      ...
      <div className="grid grid-cols-2 gap-2.5 mb-4">
        <div className="text-center py-3 rounded-xl bg-white-6">
          ...
          <div className="text-[22px] font-black text-text">{paymentCount}</div>
        </div>
        <div className="text-center py-3 rounded-xl bg-white-6">
          <div className="text-[10px] text-text-dim mb-1">Revenue</div>
          <div className={`text-[22px] font-black text-accent ${revenuePulse ? "revenue-pulse" : ""}`}>
            ${revenueCount}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div
          ...
          style={{
            opacity: dashRowStep >= 1 ? 1 : 0,
            transform: dashRowStep >= 1 ? "translateX(0)" : "translateX(-20px)",
            transition: "all 0.5s ease",
          }}
        >
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded-full bg-accent/20 flex items-center justify-center">
              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
            </div>
            <div>
              <p className="text-[11px] font-semibold text-text">Sarah M.</p>
              <p className="text-[9px] text-text-dim">Payment received</p>
            </div>
          </div>
          <span className="text-[11px] font-bold text-accent">+${price}</span>
        </div>
        ...
        <div
          className="flex items-center justify-between py-2 px-3 rounded-lg bg-white-6"
          style={{
            opacity: dashRowStep >= 2 ? 1 : 0,
            transform: dashRowStep >= 2 ? "translateX(0)" : "translateX(-20px)",
            transition: "all 0.5s ease",
          }}
        >
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded-full bg-blue-dim flex items-center justify-center">
              <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="2.5">
            ...
            <div>
              <p className="text-[11px] font-semibold text-text">Sarah M.</p>
              <p className="text-[9px] text-text-dim">Added to onboarding</p>
            </div>
          </div>
        ...
        {dashRowStep < 1 && (
          <div className="py-2.5 px-3 rounded-lg bg-white-6 flex items-center justify-center">
            <span className="text-[10px] text-text-dim">Waiting for first payment...</span>
          </div>
        )}
      </div>
    </div>
    ...
    <section className="py-20 max-md:py-14 border-t border-border">
      <div className="max-w-[1200px] mx-auto px-8 max-md:px-5">
        ...
        <div className="flex justify-center mb-4">
          <p className="text-[11px] font-bold text-text-dim uppercase tracking-widest transition-all duration-500">
            {panelLabel}
          </p>
        </div>
        ...
        <div className="relative flex justify-center" style={{ minHeight: 560 }}>
          ...
          <div
            ...
            style={{
              opacity: activePanel === "customer" ? 1 : 0,
              transform:
                activePanel === "customer"
                  ? "translateX(0) scale(1)"
                  : "translateX(-60px) scale(0.95)",
              pointerEvents: activePanel === "customer" ? "auto" : "none",
            }}
          >
            {customerPhone}
          </div>
            ...
            style={{
              ...
              transform:
                activePanel === "owner"
                  ...
                  ? "translateX(60px) scale(0.95)"
                  : "translateX(-60px) scale(0.95)",
              pointerEvents: activePanel === "owner" ? "auto" : "none",
            }}
          >
            {ownerPhone}
          </div>
          ...
          <div
            className="transition-all duration-500 ease-out absolute"
            style={{
              opacity: activePanel === "dashboard" ? 1 : 0,
              transform:
                activePanel === "dashboard"
                  ? "translateX(0) scale(1)"
                  : "translateX(60px) scale(0.95)",
              pointerEvents: activePanel === "dashboard" ? "auto" : "none",
            }}
          >
            {dashboard}
          </div>
        ...
        <div className="flex items-center justify-center gap-2 mt-6">
          {(["customer", "owner", "dashboard"] as ActivePanel[]).map((panel) => (
            <div
              key={panel}
